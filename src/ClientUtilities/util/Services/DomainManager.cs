...
namespace NUnit.Util
{
	...
	public class DomainManager : IService
	{
		...
		private static void DeleteCacheDir(DirectoryInfo cacheDir)
		{
			...
			if (cacheDir.Exists)
			{
				DirectoryInfo[] cacheDirDirectories = null;
				try
				{
					cacheDirDirectories = cacheDir.GetDirectories();
				}
				catch (Exception e)
				{
				...
				if (cacheDirDirectories != null)
				{
					foreach (DirectoryInfo dirInfo in cacheDirDirectories)
						DeleteCacheDir(dirInfo);
				}

				FileInfo[] cacheDirFiles;
				try
				{
					cacheDirFiles = cacheDir.GetFiles();
